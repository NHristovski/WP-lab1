...
public class PizzaController {
    ...
    @PostMapping
    public ResponseEntity addPizza(@Valid Pizza pizza){
        return ResponseEntity.ok(pizzaService.addPizza(pizza));
    }

    @PutMapping("/{id}")
    public ResponseEntity editPizza(@PathVariable Long id,@Valid Pizza pizza){
    ...
    @GetMapping("/compare")
    public ResponseEntity getOne(@RequestParam Long pizza1,@RequestParam Long pizza2){
